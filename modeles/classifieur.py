...
class Classifieur:
    ...
    def validation_croisee(self, x_entr, t_entr, k=10,
                            est_ech_poids=False, *args):
        """
        Cette méthode utilise simplement la méthode ``entrainement()``, qui
        est le cas le plus général pour certains modèles. Des véritables
        validations croisées propres aux modèles sont appelées directement
        dans les modèles.
        """
    ...
    def confiance_test(self, x):
        """
        Cette méthode renvoie le résultat de predict_proba ou
        decision_function, dépendant du modèle utilisé.
        """
        try:
