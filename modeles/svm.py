...
class SVM(Classifieur):
    ...
    '''
    def __init__(self, C=1.0, noyau='linear', deg=3,
                    gamma='scale', coef0=0, tol=1e-3, max_iter=-1):
        self.C = C
        self.noyau = noyau
        self.deg = deg
        self.gamma = gamma
    ...
    def validation_croisee(self, x_tab, t_tab, k=10,
        ...
        if(self.noyau == "linear" or self.noyau == "rbf"):
            ...
            for i in tqdm(range(len(liste_C))):
                self.C = liste_C[i]
                self.modele = SVC(C=self.C, kernel=self.noyau, degree=self.deg,
                                    gamma=self.gamma, coef0=self.coef0,
                                    tol=self.tol, max_iter=self.max_iter)
                for j in range(k):
                    # Masque de vrai ou de faux pour déterminer
                    # les ensembles D_valid et D_entr
                    ...
                    # aleatoirement : D_entr et D_valid
            ...
            self.C = liste_C[np.unravel_index(np.argmin(liste_erreur),
        ...
        # Noyau sigmoidal
            ...
            coef0_min = 0.00001
            coef0_max = 0.01
            liste_coef0 = np.logspace(np.log(coef0_min), np.log(coef0_max),
                                        num=15, base=np.e)
            ...
            for i in tqdm(range(len(liste_C))):
                ...
                for l in range(len(liste_coef0)):
                    ...
                    self.modele = SVC(C=self.C, kernel=self.noyau,
                                        degree=self.deg, gamma=self.gamma,
                                        coef0=self.coef0, tol=self.tol,
                    ...
                    for j in range(k):
                        ...
                        t_valid = t_tab[np.invert(liste_ind)]
                        # Entrainement sur x_entr et t_entr
                        self.entrainement(x_entr, t_entr,
                                            est_ech_poids, args[0])
                        pred_valid = self.prediction(x_valid)
            ...
            self.C = liste_C[np.unravel_index(np.argmin(liste_erreur),
                                liste_erreur.shape)[0]]
            self.coef0 = liste_coef0[np.unravel_index(np.argmin(liste_erreur),
        ...
        # Noyau polynomial
                    ...
                    for m in range(len(liste_deg)):
                        ...
                        for j in range(k):
                            ...
                            self.entrainement(x_entr, t_entr,
                                                est_ech_poids, args[0])
                            pred_valid = self.prediction(x_valid)
                            liste_erreur[i, l, m] += self.erreur(t_valid,
                                                                pred_valid)
            ...
            self.coef0 = liste_coef0[np.unravel_index(np.argmin(liste_erreur),
                                        liste_erreur.shape)[1]]
            self.deg = liste_deg[np.unravel_index(np.argmin(liste_erreur),
                                    liste_erreur.shape)[2]]
            self.modele = SVC(C=self.C, kernel=self.noyau, degree=self.deg,
                                gamma=self.gamma, coef0=self.coef0,
                                tol=self.tol, max_iter=self.max_iter)
            self.entrainement(x_tab, t_tab, est_ech_poids, args[0])
